                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                    ...
                    >
                        <div className="relative z-10 h-full flex flex-col justify-between">
                            ...
                            <div>
                                <p className="text-xs font-black uppercase tracking-[0.25em] text-white/50 mb-1">Akun Terdaftar</p>
                    ...
                    >
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-5">
                            ...
                            <div className="flex gap-4 shrink-0 bg-emerald-50 border border-emerald-100 p-5 rounded-[1.8rem] items-center">
                                ...
                                <div>
                                    <p className="text-xs font-black uppercase tracking-widest text-emerald-600/70 mb-0.5">Status Sistem</p>
                ...
                <div className="bg-white/70 backdrop-blur-3xl rounded-xl border border-white/60 shadow-[0_20px_50px_rgb(0,0,0,0.05)] overflow-hidden">
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-100/50 border-b border-white/40">
                                    <th className="px-5 py-6 text-xs font-black uppercase tracking-[0.2em] text-slate-500 cursor-pointer hover:text-primary-600 transition-colors" onClick={() => handleSort('nip')}>
                                        <div className="flex items-center gap-2">Identitas <ArrowUpDown size={12} className="opacity-50" /></div>
                                    </th>
                                    <th className="px-5 py-6 text-xs font-black uppercase tracking-[0.2em] text-slate-500">Otoritas</th>
                                    <th className="px-5 py-6 text-xs font-black uppercase tracking-[0.2em] text-slate-500">Keamanan</th>
                                    <th className="px-5 py-6 text-xs font-black uppercase tracking-[0.2em] text-slate-500 cursor-pointer hover:text-primary-600 transition-colors" onClick={() => handleSort('status')}>
                                        <div className="flex items-center gap-2">Status <ArrowUpDown size={12} className="opacity-50" /></div>
                                    </th>
                                    <th className="px-5 py-6 text-xs font-black uppercase tracking-[0.2em] text-slate-500">Aktivitas Terakhir</th>
                                    <th className="px-5 py-6 text-xs font-black uppercase tracking-[0.2em] text-slate-500 text-right">Aksi</th>
                                </tr>
                            </thead>
                                ...
                                {loading ? (
                                    Array(5).fill(0).map((_, i) => (
                                        <tr key={i} className="animate-pulse">
                                            <td colSpan={6} className="px-5 py-8"><div className="h-4 bg-slate-200/50 rounded-full w-full"></div></td>
                                        </tr>
                                    ))
                                ) : users.map((user, idx) => {
                                        ...
                                        >
                                            ...
                                            <td className="px-5 py-6">
                                                {user.role === 'super admin' ? (
                                                    <span className="inline-flex items-center gap-1.5 rounded-xl bg-slate-900 px-3 py-1.5 text-xs font-black text-white uppercase tracking-widest shadow-sm">
                                                        <Shield size={12} className="text-primary-400" /> Admin
                                                    </span>
                                                ) : (
                                                    <span className="inline-flex items-center gap-1.5 rounded-xl bg-white px-3 py-1.5 text-xs font-black text-slate-600 uppercase tracking-widest border border-slate-200 shadow-sm">
                                                        Personil
                                                    </span>
                                                )}
                                            </td>
                                            <td className="px-5 py-6">
                                                {user.mfa_enabled ? (
                                                    <span className="inline-flex items-center gap-1.5 rounded-xl bg-emerald-50 px-3 py-1.5 text-xs font-black text-emerald-700 uppercase tracking-widest border border-emerald-100">
                                                        <ShieldCheck size={14} /> 2FA Aktif
                                                    </span>
                                                ) : (
                                                    <span className="inline-flex items-center gap-1.5 rounded-xl bg-slate-50 px-3 py-1.5 text-xs font-black text-slate-400 uppercase tracking-widest border border-slate-200">
                                                        <Shield size={14} className="opacity-40" /> Non-Aktif
                                                    </span>
                                                )}
                                            </td>
                                            <td className="px-5 py-6">
                                                <span className={`inline-flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-xs font-black uppercase tracking-widest ${status.color.replace('ring-', 'border-').replace('/20', '')}`}>
                                            ...
                                            <td className="px-5 py-6">
                                                {user.last_activity_at ? (
                                                    ...
                                                    <span className="text-slate-300 italic font-bold text-xs uppercase tracking-widest">Belum Aktif</span>
                                                )}
                                            </td>
                    ...
                    {pagination && pagination.total_pages > 1 && (
                        <div className="px-5 py-6 border-t border-slate-100/50 flex items-center justify-between bg-slate-50/30">
                            <span className="text-xs font-black text-slate-500 uppercase tracking-[0.2em]">Halaman <span className="text-primary-600">{page}</span> dari {pagination.total_pages}</span>
                            <div className="flex items-center gap-2">
                                <button
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md">
                        ...
                        >
                            <div className="bg-slate-900 px-4 py-6 text-white text-center relative overflow-hidden border-b border-white/5">
                                ...
                                <h3 className="text-xl font-black italic tracking-tight relative z-10 uppercase">Konfigurasi Akses</h3>
                                <p className="text-primary-400 text-xs font-black uppercase tracking-[0.3em] mt-2 relative z-10 italic">Manajemen Izin Sistem</p>
                            </div>
                            <div className="p-4 space-y-8">
                                <div className="flex items-center gap-5 p-5 rounded-lg bg-slate-50 border border-slate-100 shadow-inner">
                                ...
                                <div className="space-y-6">
                                    <div className="space-y-3">
                                        <label className="text-xs font-black text-slate-900 uppercase tracking-widest block pl-1">Status Akun</label>
                                    ...
                                    <div className="space-y-3">
                                        <label className="text-xs font-black text-slate-900 uppercase tracking-widest block pl-1">Level Otoritas</label>
                                        <div className="relative hover:shadow-sm transition-shadow rounded-2xl">
                                            <select
                                                className="w-full bg-slate-50 border-2 border-slate-50 focus:border-primary-500 focus:bg-white rounded-2xl px-4 py-4 text-sm font-black text-slate-800 transition-all outline-none appearance-none cursor-pointer"
                                                value={newRole}
                                    ...
                                    {editingUser.mfa_enabled && (
                                        <div className="pt-4 border-t border-slate-100">
                                            <label className="text-xs font-black text-rose-600 uppercase tracking-widest flex items-center gap-2 mb-3">
                                                <ShieldAlert size={14} /> Keamanan Akun Lapisan Ganda
                                            </label>
                                            <div className="p-5 rounded-lg bg-rose-50 border border-rose-100 flex items-center justify-between shadow-inner">
                                                <div>
                                                    <p className="text-xs font-black uppercase text-rose-700 tracking-widest mb-1">MFA Sedang Aktif</p>
                                                    <p className="text-xs text-rose-500 font-bold">Opsi reset untuk kehilangan akses.</p>
                                                </div>
                                                <button
                                                    onClick={handleResetMFA}
                                                    disabled={saving}
                                                    className="px-4 py-2.5 bg-white text-rose-600 text-xs font-black uppercase tracking-widest rounded-xl border border-rose-200 hover:bg-rose-600 hover:text-white transition-all shadow-sm active:scale-95"
                                                >
                                                    MFA Reset
                                                </button>
                                            </div>
                                ...
                                <div className="flex gap-4 pt-4">
                                    <button onClick={() => setShowEditModal(false)} className="flex-1 py-5 rounded-2xl bg-white border-2 border-slate-200 text-slate-600 font-black text-xs uppercase tracking-widest hover:bg-slate-50 transition-all">Batalkan</button>
                                    <button onClick={handleUpdate} className="flex-[2] py-5 rounded-3xl bg-slate-900 text-white font-black text-xs uppercase tracking-widest hover:bg-primary-600 transition-all shadow-2xl shadow-primary-100 flex items-center justify-center gap-3 active:scale-95" disabled={saving}>
                    ...
                    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md">
                        ...
                        >
                            <div className="bg-rose-600 px-4 py-6 text-white text-center relative overflow-hidden">
                                <div className="h-16 w-16 rounded-[2.2rem] bg-white/20 backdrop-blur-md flex items-center justify-center mx-auto mb-6 border-2 border-white/30 shadow-2xl relative z-10"><AlertTriangle size={40} strokeWidth={2.5} /></div>
                                <h3 className="text-xl font-black italic tracking-tighter relative z-10 uppercase">HAPUS AKUN</h3>
                                <p className="text-rose-200 text-xs font-black uppercase tracking-[0.3em] mt-2 relative z-10 font-bold">Peringatan Penghapusan Data</p>
                            </div>
                            <div className="p-4 space-y-8">
                                ...
                                <div className="flex flex-col gap-3 pt-4">
                                    ...
                                    <button onClick={() => { setShowDeleteModal(false); setDeletingUser(null); }} className="w-full py-2 text-slate-400 font-black text-xs uppercase tracking-widest hover:text-slate-900 transition-colors">Batalkan Operasi</button>
                                </div>
                            </div>
                        </motion.div>
                    </div>
