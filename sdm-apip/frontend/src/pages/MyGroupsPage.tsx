    ...
    return (
        ...
        >
            <div className="space-y-8 animate-fade-in">
                {/* Search Bar Bento */}
                <div className="bg-white/70 backdrop-blur-3xl p-4 rounded-lg border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                    <div className="relative max-w-xl">
                        ...
                        <input
                            ...
                            className="w-full pl-14 pr-6 py-4 bg-white/50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all outline-none font-bold text-slate-700 placeholder:text-slate-500"
                            value={searchTerm}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredGroups.map(group => (
                        <motion.button
                            ...
                            className="group card p-5 text-left transition-all border-white/60 shadow-[0_8px_25px_rgb(0,0,0,0.03)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] bg-white/70 backdrop-blur-3xl"
                        >
                            <div className="flex items-start justify-between mb-6">
                                <div className="h-16 w-16 rounded-md bg-primary-50 text-primary-600 flex items-center justify-center group-hover:bg-primary-600 group-hover:text-white group-hover:rotate-3 transition-all duration-500 shadow-sm">
                                ...
                                <div className="flex flex-col items-end gap-3">
                                    ...
                                    {group.user_role === 'Ketua' && (
                                        <span className="px-3 py-1 rounded-full bg-amber-50 text-xs font-black text-amber-600 uppercase tracking-widest border border-amber-100 shadow-sm transition-transform group-hover:scale-110">Ketua Tim</span>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="pt-6 border-t border-slate-100/50 flex items-center justify-between mt-auto">
                                <span className="text-xs font-black uppercase tracking-widest text-slate-400 group-hover:text-primary-600 transition-colors flex items-center gap-2">
                    ...
                    {filteredGroups.length === 0 && (
                        <div className="col-span-full py-6 card text-center border-dashed bg-white/40 shadow-none border-2 border-slate-200">
            ...
            {showGroupDetailModal && selectedGroupDetail && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md animate-fade-in">
                    ...
                    >
                        <div className="px-4 py-8 border-b border-slate-100 flex items-center justify-between bg-white/50 sticky top-0 z-10">
                            <div>
                                <h3 className="text-xl font-black text-slate-900 tracking-tight">{selectedGroupDetail.group.name}</h3>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                            <div className="bg-primary-50/50 rounded-lg p-4 mb-10 border border-primary-100/50 flex flex-col md:flex-row md:items-center justify-between gap-6">
                                <div className="space-y-1">
                                    <p className="text-xs font-black text-primary-500 uppercase tracking-widest">Deskripsi Grup</p>
                                ...
                                <div className="bg-white/80 px-4 py-4 rounded-3xl border border-white shadow-sm shrink-0 text-center">
                                    <p className="text-xl font-black text-primary-600 leading-none mb-1">{selectedGroupDetail.members?.length || 0}</p>
                                    <p className="text-xs font-black text-slate-400 uppercase tracking-tight">SDM APIP</p>
                                </div>
                            </div>
                                    ...
                                    >
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <div className="flex items-center gap-2 mb-0.5">
                                                ...
                                                {member.group_role === 'Ketua' && (
                                                    <div className="px-2 py-0.5 rounded-lg bg-amber-50 text-xs font-black text-amber-600 uppercase border border-amber-100">Tim Leader</div>
                                                )}
                                            </div>
                                            <p className="text-xs text-slate-500 font-bold truncate">{member.jabatan || 'Anggota APIP'}</p>
                                            <div className="mt-1.5 flex items-center gap-2">
                                                <span className="text-xs font-black text-slate-300 bg-slate-50 px-2.5 py-1 rounded-lg border border-slate-100 group-hover/item:text-primary-400 transition-colors">{member.nip}</span>
                                            </div>
                                        </div>
                                    </motion.div>
                                ))}
                            ...
                            <div className="mt-12 bg-primary-900/5 rounded-xl p-5 border border-primary-100 flex items-start gap-4">
