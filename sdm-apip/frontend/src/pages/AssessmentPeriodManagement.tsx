...
const AssessmentPeriodManagement: React.FC = () => {
    ...
    const getStatusBadge = (period: AssessmentPeriod) => {
        if (isPeriodExpiredByDate(period)) {
            return (
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-2xl bg-rose-50 border border-rose-100 text-xs font-black text-rose-700 uppercase tracking-widest">
        ...
        if (isPeriodUpcoming(period)) {
            return (
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-2xl bg-primary-50 border border-primary-100 text-xs font-black text-primary-700 uppercase tracking-widest">
                    Akan Datang
                </div>
            );
        }
        if (period.is_active) {
            return (
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-2xl bg-emerald-50 border border-emerald-100 text-xs font-black text-emerald-700 uppercase tracking-widest shadow-sm">
                    <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" /> Berjalan
                </div>
            );
        }
        return (
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-2xl bg-slate-100 border border-slate-200 text-xs font-black text-slate-500 uppercase tracking-widest">
                Nonaktif
            </div>
        );
    };
            ...
            <div className="space-y-8 animate-fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                    <motion.div
                        ...
                        className="md:col-span-8 bg-white/70 backdrop-blur-3xl rounded-xl p-5 border border-white/60 shadow-[0_8px_30px_rgba(0,0,0,0.04)] flex items-start gap-6 relative overflow-hidden group"
                    >
                        <div className="absolute top-0 right-0 p-5 opacity-5 scale-150 rotate-12 transition-transform duration-1000 group-hover:rotate-0">
                            <Calendar size={120} />
                        </div>
                        <div className="h-14 w-14 rounded-md bg-primary-50 text-primary-600 flex items-center justify-center shrink-0 shadow-sm border border-primary-100/50">
                    ...
                    <motion.div
                        ...
                        className="md:col-span-4 bg-slate-900 rounded-xl p-5 flex flex-col justify-between shadow-2xl relative overflow-hidden border border-slate-800"
                    >
                        <div className="absolute top-0 right-0 p-5 opacity-10">
                            <Plus size={80} className="text-white" />
                        </div>
                        <p className="text-xs font-black text-white/40 uppercase tracking-[0.3em] mb-4">Administrasi</p>
                        <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    {loading ? (
                        Array(3).fill(0).map((_, i) => (
                            <div key={i} className="bg-white/50 backdrop-blur-sm rounded-xl h-[320px] animate-pulse border border-white/50" />
                        ))
                    ) : periods.length === 0 ? (
                        <div className="col-span-full py-32 text-center bg-white/40 backdrop-blur-3xl rounded-2xl border-4 border-dashed border-white/60">
                            ...
                            <p className="text-slate-400 font-bold uppercase text-xs tracking-[0.4em] mt-3">Silakan buat periode penilaian baru</p>
                        </div>
                    ) : (
                        periods.map((period, idx) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    className={`bg-white/70 backdrop-blur-3xl rounded-[2.8rem] border border-white/60 p-5 shadow-[0_15px_45px_rgba(0,0,0,0.04)] hover:shadow-[0_25px_60px_rgba(0,0,0,0.06)] transition-all flex flex-col justify-between group h-full relative overflow-hidden ${expired ? 'grayscale-[0.5] opacity-80' : ''}`}
                                >
                                    {nearDeadline && (
                                        <div className="absolute top-0 right-0 px-4 py-2 bg-amber-500 text-white text-xs font-black uppercase tracking-widest rounded-bl-3xl shadow-lg animate-pulse">
                                            Urgent Deadline
                                        </div>
                                    )}
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs font-black text-primary-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                                                <RefreshCw size={10} strokeWidth={3} /> {period.frequency.replace('_', ' ')}
                                            </p>
                                        </div>

                                        <div className="space-y-3">
                                            <div className="flex items-center gap-3 text-slate-600 bg-slate-100/50 p-4 rounded-2xl border border-slate-100/50 shadow-inner">
                                            ...
                                            {!expired && !upcoming && days >= 0 && (
                                                <div className={`text-xs font-black tracking-[0.2em] uppercase px-4 py-2 rounded-xl text-center w-full transition-colors ${nearDeadline ? 'bg-rose-500 text-white animate-bounce' : 'bg-emerald-50 text-emerald-600 border border-emerald-100'}`}>
                                                    {days === 0 ? 'HARI TERAKHIR' : `SISA ${days} HARI LAGI`}
                                                </div>
                                            )}

                                            {expired && (
                                                <div className="text-xs font-black tracking-[0.2em] uppercase px-4 py-2 rounded-xl text-center w-full bg-slate-800 text-white italic">
                                                    SYSTEM LOCKED
                                                </div>
                                            )}
                                        </div>
                                    ...
                                    <div className="mt-10 pt-6 border-t border-slate-100 flex items-center justify-between">
                                        {expired ? (
                                            <button
                                                ...
                                                className="flex items-center gap-2.5 px-5 py-2.5 rounded-2xl bg-amber-500 text-white text-xs font-black uppercase tracking-widest hover:bg-amber-600 transition-all shadow-lg shadow-amber-200 active:scale-95"
                                            >
                                            ...
                                            <button
                                                onClick={() => handleToggleStatus(period)}
                                                className={`flex items-center gap-2.5 px-5 py-2.5 rounded-2xl text-xs font-black uppercase tracking-widest transition-all active:scale-95 border ${period.is_active ? 'bg-primary-600 text-white border-primary-600 shadow-lg shadow-primary-100' : 'bg-white text-slate-400 border-slate-200'}`}
                                            >
                                            ...
                                            <div className="flex items-center gap-2.5 px-5 py-2.5 rounded-2xl bg-emerald-50 text-emerald-600 text-xs font-black uppercase tracking-widest border border-emerald-100">
            ...
            <AnimatePresence>
                {overrideConfirm && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md">
                        ...
                        >
                            <div className="bg-amber-500 px-5 py-8 text-white text-center relative border-b border-white/5">
                                ...
                                <p className="text-amber-100 text-xs font-black uppercase tracking-[0.3em] mt-2">Daftar Audit Keamanan Diperlukan</p>
                                <button onClick={() => setOverrideConfirm(null)} className="absolute top-4 right-4 h-12 w-12 flex items-center justify-center rounded-2xl bg-white/10 hover:bg-rose-500 transition-all group active:scale-90 z-20">
                            ...
                            <div className="p-4 space-y-6">
                                ...
                                <div className="bg-slate-900 rounded-3xl p-4 text-white space-y-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-xs font-black uppercase tracking-widest text-white">RESIKO AKSES</span>
                                    </div>
                                ...
                                <div className="flex flex-col gap-3 pt-4">
                                    ...
                                    <button onClick={() => setOverrideConfirm(null)} className="w-full py-2 text-slate-400 font-black text-xs uppercase tracking-widest hover:text-slate-900 transition-colors">Batal</button>
                                </div>
                            </div>
                        </motion.div>
                    </div>
                ...
                {deleteConfirm && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md">
                        ...
                        >
                            <div className="bg-rose-600 p-4 text-white text-center relative border-b border-white/5">
                                <div className="h-16 w-16 rounded-[2.2rem] bg-white text-rose-600 flex items-center justify-center mx-auto mb-6 shadow-2xl">
                                    <AlertCircle size={48} strokeWidth={2.5} />
                                </div>
                                <h3 className="text-2xl font-black italic tracking-tight uppercase">ELIMINASI PERIODE</h3>
                                <p className="text-rose-200 text-xs font-black uppercase tracking-[0.3em] mt-2">Peringatan: Data akan hancur permanen</p>
                                <button onClick={() => setDeleteConfirm(null)} className="absolute top-4 right-4 h-12 w-12 flex items-center justify-center rounded-2xl bg-white/10 hover:bg-slate-900 transition-all group active:scale-90 z-20">
                                    <X size={20} strokeWidth={3} className="group-hover:rotate-90 transition-transform" />
                                </button>
                            </div>
                            <div className="p-4 space-y-6">
                                ...
                                <div className="flex flex-col gap-3 pt-6">
                                    ...
                                    <button onClick={() => setDeleteConfirm(null)} className="w-full py-2 text-slate-400 font-black text-xs uppercase tracking-widest hover:text-slate-900 transition-colors" disabled={deleting}>Gagalkan</button>
                                </div>
                            </div>
                        </motion.div>
                    </div>
                ...
                {showEditModal && editPeriod && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md">
                        ...
                        <motion.div
                            ...
                            className="relative bg-white w-full max-w-lg rounded-xl shadow-[0_30px_100px_rgba(0,0,0,0.4)] overflow-hidden flex flex-col max-h-[90vh]"
                        >
                            <div className="px-5 py-6 border-b border-slate-100 flex items-center justify-between shrink-0 bg-slate-100/50">
                                <div>
                            ...
                            <div className="p-5 overflow-y-auto custom-scrollbar">
                                <form id="editPeriodForm" onSubmit={handleUpdate} className="space-y-6">
                                    <div className="space-y-3">
                                        <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-2">Nama Periode</label>
                                        <input
                                            type="text"
                                            required
                                    ...
                                    <div className="grid grid-cols-2 gap-6">
                                        <div className="space-y-3">
                                            <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-2">Tanggal Mulai</label>
                                            <input
                                                type="date"
                                                required
                                                className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-5 py-4 text-sm font-bold focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all outline-none"
                                        ...
                                        <div className="space-y-3">
                                            <label className="text-xs font-black text-primary-500 uppercase tracking-widest ml-2">Tenggat Waktu / Selesai</label>
                                            <input
                                                type="date"
                                                required
                                            ...
                                            />
                                            <p className="text-xs font-bold text-primary-400 mt-2 px-2">Anda bisa mengubah End Date semaunya untuk memberikan waktu tambahan spesifik.</p>
                                        </div>
                                    </div>

                                    <div className="space-y-3">
                                        <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-2">Frekuensi</label>
                                        <input
                                            type="text"
                                            disabled
                                        ...
                                        />
                                        <p className="text-xs font-bold text-slate-400 mt-2 px-2">*Frekuensi tidak dapat diubah setelah dibuat karena terikat struktur porsi penilaian.</p>
                                    </div>
                                </form>
                            </div>

                            <div className="p-5 border-t border-slate-100 bg-slate-100/50 shrink-0 flex items-center justify-end gap-3">
                                <button type="button" onClick={() => setShowEditModal(false)} className="px-4 py-3.5 rounded-xl font-bold text-slate-500 hover:bg-slate-200 transition-colors text-sm">
                                    Batal
                                </button>
                                <button type="submit" form="editPeriodForm" className="px-5 py-3.5 rounded-xl font-black text-white bg-primary-600 hover:bg-primary-700 shadow-lg shadow-primary-200 transition-all active:scale-95 text-sm uppercase tracking-widest">
                                    Simpan Perubahan
                                </button>
                            </div>
                        </motion.div>
                ...
                {showModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-md">
                        ...
                        <motion.div
                            ...
                            className="relative bg-white w-full max-w-lg rounded-2xl shadow-[0_30px_100px_rgba(0,0,0,0.3)] overflow-hidden border border-white/40"
                        >
                            <div className="bg-slate-900 px-4 py-6 text-white flex items-center justify-between relative border-b border-white/5">
                                <div className="relative z-10">
                                    <h3 className="text-xl font-black italic tracking-tight">BUAT PERIODE BARU</h3>
                                    <p className="text-primary-400 text-xs font-black uppercase tracking-[0.3em] mt-2">Daftarkan Siklus Penilaian Baru</p>
                                </div>
                                <button onClick={() => setShowModal(false)} className="relative z-10 h-10 w-10 flex items-center justify-center rounded-xl bg-white/10 hover:bg-rose-500 transition-all active:scale-90">
                                    <X size={20} strokeWidth={3} className="group-hover:rotate-90 transition-transform" />
                                </button>
                            </div>
                            <form onSubmit={handleCreate} className="p-4 space-y-8">
                                <div className="space-y-3">
                                    <label className="text-xs font-black text-slate-900 uppercase tracking-widest block pl-1">Nama Periode</label>
                                    <input
                                        type="text"
                                ...
                                <div className="space-y-3">
                                    <label className="text-xs font-black text-slate-900 uppercase tracking-widest block pl-1">Frekuensi Penilaian</label>
                                    <select
                                        value={newPeriod.frequency}
                                        onChange={(e) => setNewPeriod({ ...newPeriod, frequency: e.target.value })}
                                        className="w-full bg-slate-50 border-2 border-slate-50 focus:border-primary-500 focus:bg-white rounded-2xl px-4 py-4 text-sm font-black text-slate-800 transition-all outline-none appearance-none cursor-pointer"
                                        required
                                    >
                                ...
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="space-y-3">
                                        <label className="text-xs font-black text-slate-900 uppercase tracking-widest block pl-1">Tanggal Mulai</label>
                                        <input
                                            type="date"
                                    ...
                                    <div className="space-y-3">
                                        <label className="text-xs font-black text-slate-900 uppercase tracking-widest block pl-1">Tanggal Selesai</label>
                                        <input
                                            type="date"
