...
func (gc *GroupController) mapServiceError(c *gin.Context, err error, action string) {
	...
	case services.ErrInternalServer:
		utils.ErrorResponse(c, http.StatusInternalServerError, action, "Terjadi kesalahan pada server. Silakan hubungi administrator jika masalah berlanjut.")
	default:
		// Generic fallback for safety
		utils.ErrorResponse(c, http.StatusInternalServerError, action, "Terjadi kesalahan yang tidak terduga. Silakan coba beberapa saat lagi.")
	}
}
